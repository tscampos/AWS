                                    # Cria Gateway para acesso a internet* (*AINDA PRECISA DE OUTRAS CONFIGURAÇÕES)
                                    ## Anexa Gateway à VPC criada (vpc.tf)
...
resource "aws_subnet" "private" {
  ...
  tags = {
    Name        = "${var.app_name}-private-subnet-${count.index + 1}" # [1] agroforte-private-subnet-10.10.0.0/24 | [2] {...}-10.10.1.0/24
    Environment = var.app_environment
  }
}

                                    # Cria sub-rede PÚPLICA
